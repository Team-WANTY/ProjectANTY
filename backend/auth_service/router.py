...
from auth_service.main import get_auth_service
from auth_service.models import UserBase
...
@auth_router.post(
    "/register", response_model=UserBase, status_code=status.HTTP_201_CREATED
)
async def register(
    response: Response,
    ...
    try:
        ...
        raise HTTPException(
            status_code=e.response.status_code, detail=e.response.detail
        )
    except AuthInterserviceError as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Interservice error: {e}",
        )
    except Exception:
        # TODO add logging
        raise HTTPException(
