...
from backend.exceptions.user import UserUsernameExistsError, UserEmailExistsError
from backend.exceptions.auth import AuthIncorrectPasswordError
import pytest
...
class TestAuthRoutes:
    ...
    async def test_login_success(self, async_client, sample_user, mock_auth_service):
        ...
        response = await async_client.post(
            "/auth/login", data={"username": sample_user.username, "password": "TestPassword123!"}
        )
    ...
    async def test_login_invalid_credentials(self, async_client, sample_user, mock_auth_service):
        ...
        mock_auth_service.authenticate_user_by_username.side_effect = AuthIncorrectPasswordError()

        response = await async_client.post(
            "/auth/login", data={"username": sample_user.username, "password": "WrongPassword"}
        )
