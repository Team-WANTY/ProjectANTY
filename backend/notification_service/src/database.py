...
class NotificationsDB:
    ...
    async def update_notification(self, notification_id:str):
        ...
        try:
            ...
            logger.debug(
                f"Trying to send update operations to DB for notification with ID '{post_update.id}'"
            )
    ...
    async def delete_notification(self, notification_id: str):
        try:
            ...
            logger.debug(f"Successfully deleted notification with ID '{notification_id}'")
        except exceptions.CosmosResourceNotFoundError:
            logger.warning(f"Error while deleting notification '{notification_id}', not found")
            raise RecordNotFoundError()
        except Exception as e:
            logger.error(f"Error while deleting notification '{notification_id}', unexpected: {e}")
