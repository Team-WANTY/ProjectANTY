...
async def create_notification(
    ...
    try:
        ...
        logger.error(f"Error creating notification: {new_notification.model_dump()}: already exists")
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Exists already")
    except RecordCreationError:
        logger.error(f"Error creating notification: {new_notification.model_dump()}: creation error")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    except Exception as e:
        logger.error(f"Error creating notification: {new_notification.model_dump()}, unexpected: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
async def get_notification_by_id(
    ...
    try:
        ...
        logger.warning(f"Error creating post: {new_post.model_dump()}: authorization error")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    except RecordNotFoundError:
        logger.error(f"Error getting notification with ID '{notification_id}': not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    except GeneralQueryError:
        logger.error(f"Error getting notification with ID '{notification_id}': query error")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    except Exception as e:
        logger.error(f"Error getting notification with ID '{notification_id}', unexpected: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
async def get_users_unread_notification_ids(
    ...
    try:
        return await service.get_users_unread_notification_ids(
            user_id, getter, max_items, continuation_token
        )
    except AuthError:
        logger.warning(f"Error creating post: {new_post.model_dump()}: authorization error")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
...
async def read_notification(
    ...
    try:
        ...
        logger.error(f"Error updating notification with ID '{notification_id}': not authorized")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    except RecordNotFoundError:
        logger.error(f"Error updating notification with ID '{notification_id}': not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    except RecordUpdateError:
        logger.error(f"Error updating notification with ID '{notification_id}': query error")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    except Exception as e:
        logger.error(f"Error updating notification with ID '{notification_id}', unexpected: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)