...
from backend.security.token import decode_token
from backend.dependencies import get_users_service
...
async def get_current_user(token: str = Depends(oauth2_scheme), users_service:UsersService=Depends(get_users_service)) -> User:
...
async def get_user(user_id: str, users_service:UsersService=Depends(get_users_service)):
    """Get user by ID"""
    try:
...
async def delete_user(
    user_id: str, current_user: User = Depends(get_current_user), users_service:UsersService=Depends(get_users_service)
):
