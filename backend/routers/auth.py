...
from backend.dependencies import get_users_service, get_auth_service
...
async def register(response:Response, user_create: UserCreate, users_service:UsersService=Depends(get_users_service)):
    """Register a new user"""
    try:
...
async def login(response: Response, form_data: OAuth2PasswordRequestForm = Depends(), auth_service:AuthService=Depends(get_auth_service)):
...
async def refresh_token(request: Request, users_service:UsersService=Depends(get_users_service)):
